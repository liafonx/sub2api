export default {
  ...
  admin: {
    ...
    users: {
      ...
      rpmLimitMin: "RPM 限制不能小于 0（0 = 无限制）",
    ...
    groups: {
      ...
      userAccountAffinity: {
        title: "用户每日账号绑定",
        tooltip:
          "启用后，同一用户在一天内将固定使用同一个账号（在配置的 UTC 时间重置）。仅当该账号费用超限时才会自动切换。",
        enabled: "已开启",
        disabled: "未开启",
      },
      openaiMessages: {
    ...
    accounts: {
      ...
      capacity: {
        ...
        userQuota: {
          active: "{count} 个活跃用户，每用户限额：{limit}",
          inactive: "用户配额未启用",
        },
      },
      ...
      quotaLimitHint:
        "设置日/周/总使用额度（美元），任一维度达到限额后账号暂停调度。Anthropic API Key 账号还可配置客户端绑定。修改限额不会重置已用额度。",
      ...
      quotaControl: {
        ...
        rpmLimit: {
          ...
          stickyBufferHint:
            "超过 基础 RPM 后，粘性会话额外允许的请求数。为空则使用默认值（基础 RPM 的 20%，最小为 1）",
          userMsgQueue: "用户消息限速",
        ...
        dynamicCost: {
          label: "动态费用追踪",
          hint: "使用 Anthropic 实时账单数据替代本地 token 估算",
          fallbackNote: "账单数据不可用时，回退到本地 token 估算。",
          limit7d: "7天消费限额",
          limit7dPlaceholder: "100",
          limit7dHint: "7天消费达到此限额后暂停调度（美元）",
          reserve7d: "7天粘性预留",
          reserve7dPlaceholder: "20",
          reserve7dHint: "超出7天限额后为粘性会话预留的额外预算（美元）",
        },
        userQuota: {
          label: "用户配额分配",
          hint: "将窗口预算平均分配给活跃用户，防止单个用户耗尽账号额度",
          requiresWindowCost: "需要启用窗口费用控制",
          idleTimeout: "用户空闲超时（秒）",
          idleTimeoutPlaceholder: "60",
          idleTimeoutHint: "用户空闲超过此时间后从活跃集合中移除",
        },
        userRPM: {
          label: "用户 RPM 分配",
          hint: "将账号基础 RPM 平均分配给活跃用户",
          requiresRPM: "需要设置 基础 RPM",
        },
      },
      affinityNoClients: "无绑定客户端",
      affinityClients: "{count} 个绑定客户端：",
      affinitySection: "客户端绑定",
      affinitySectionHint:
        "控制客户端在账号间的分布。通过配置区域阈值来平衡负载。",
      affinityToggle: "启用客户端绑定",
    ...
    settings: {
      ...
      defaults: {
        ...
        defaultRPMLimit: "默认 RPM 限制",
        defaultRPMLimitHint: "新用户的每分钟最大请求数（0 = 无限制）",
