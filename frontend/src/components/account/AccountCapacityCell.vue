<template>
  <div class="flex flex-col gap-0.5">
    ...
    <!-- 5h窗口费用限制 -->
    <CapacityBadge v-if="showWindowCost" :color-class="windowCostClass" :tooltip="windowCostTooltip" :current="'$' + formatCost(currentWindowCost)" :max="'$' + formatCost(effective5hLimit)">
      <svg class="h-2.5 w-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
// Effective 5h limit: prefer derived, fall back to manual (Patch 19: dynamic cost)
const effective5hLimit = computed(() => {
  return props.account.effective_5h_limit ?? props.account.window_cost_limit ?? 0
})

// 每用户配额 — compute from real-time cost data when available to avoid stale cache lag (Patch 3)
