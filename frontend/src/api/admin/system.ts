...
export async function getCCProbeStatus(): Promise<CCProbeTraits | null> {
  const { data } = await apiClient.get<CCProbeTraits | null>('/admin/system/cc-probe')
  return data ?? null
}
...
export async function getCCProbeConfig(): Promise<CCProbeConfig | null> {
  const { data } = await apiClient.get<CCProbeConfig | null>('/admin/system/cc-probe/config')
  return data ?? null
}
...
export async function triggerCCProbe(): Promise<CCProbeTraits> {
  const { data } = await apiClient.post<CCProbeTraits>('/admin/system/cc-probe/trigger')
  return data
}
