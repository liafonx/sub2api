...
export async function getSoraS3Settings(): Promise<SoraS3Settings> {
  const { data } = await apiClient.get<SoraS3Settings>(
    "/admin/settings/sora-s3",
  );
  return data;
}
...
export async function updateSoraS3Settings(
  ...
  const { data } = await apiClient.put<SoraS3Settings>(
    "/admin/settings/sora-s3",
    settings,
  );
  return data;
...
export async function listSoraS3Profiles(): Promise<ListSoraS3ProfilesResponse> {
  const { data } = await apiClient.get<ListSoraS3ProfilesResponse>(
    "/admin/settings/sora-s3/profiles",
  );
  return data;
}
...
export async function setActiveSoraS3Profile(
  profileID: string,
): Promise<SoraS3Profile> {
  const { data } = await apiClient.post<SoraS3Profile>(
