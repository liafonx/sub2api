...
**Bugfix (2026-04-14)**: Affinity resolved correctly but requests still ping-ponged between accounts. Root cause: `isAccountInGroup` in Layer 1.5 always returned `false` for affinity-resolved accounts because `accountByID` was built from `ListSchedulableAccounts()` → `GetSnapshot()` which reads the metadata cache (`sched:meta:{id}`). The `buildSchedulerMetadataAccount` function intentionally omits `AccountGroups` and `GroupIDs` to save cache space, so the group membership check failed. Fixed by: (1) adding `GroupIDs` to `buildSchedulerMetadataAccount` in `repository/scheduler_cache.go`; (2) updating `isAccountInGroup` in `service/gateway_service.go` to check both `AccountGroups` (full accounts) and `GroupIDs` (metadata-only snapshot accounts).
