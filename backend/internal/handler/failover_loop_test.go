...
func TestHandleFailoverError_BasicSwitch(t *testing.T) {
	t.Run("非重试错误_非Antigravity_直接切换", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(context.Background(), mock, nil, 100, "openai", err)
	...
	t.Run("非重试错误_Antigravity_第一次切换无延迟", func(t *testing.T) {
		...
		start := time.Now()
		action := fs.HandleFailoverError(context.Background(), mock, nil, 100, service.PlatformAntigravity, err)
		elapsed := time.Since(start)
	...
	t.Run("非重试错误_Antigravity_第二次切换有1秒延迟", func(t *testing.T) {
		...
		err := newTestFailoverErr(500, false, false)
		start := time.Now()
		action := fs.HandleFailoverError(context.Background(), mock, nil, 200, service.PlatformAntigravity, err)
		elapsed := time.Since(start)
	...
	t.Run("连续切换直到耗尽", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(context.Background(), mock, nil, 100, "openai", err1)
		require.Equal(t, FailoverContinue, action)
		require.Equal(t, 1, fs.SwitchCount)

		// 第二次切换：1→2
		err2 := newTestFailoverErr(502, false, false)
		action = fs.HandleFailoverError(context.Background(), mock, nil, 200, "openai", err2)
		require.Equal(t, FailoverContinue, action)
		require.Equal(t, 2, fs.SwitchCount)

		// 第三次已耗尽：SwitchCount(2) >= MaxSwitches(2)
		err3 := newTestFailoverErr(503, false, false)
		action = fs.HandleFailoverError(context.Background(), mock, nil, 300, "openai", err3)
...
func TestHandleFailoverError_CacheBilling(t *testing.T) {
	t.Run("hasBoundSession为true时设置ForceCacheBilling", func(t *testing.T) {
		...
		fs.HandleFailoverError(context.Background(), mock, nil, 100, "openai", err)
	...
	t.Run("一旦设置不会被后续错误重置", func(t *testing.T) {
		...
		fs.HandleFailoverError(context.Background(), mock, nil, 100, "openai", err1)
		require.True(t, fs.ForceCacheBilling)

		// 第二次：ForceCacheBilling=false → 仍然保持 true
		err2 := newTestFailoverErr(502, false, false)
		fs.HandleFailoverError(context.Background(), mock, nil, 200, "openai", err2)
...
func TestHandleFailoverError_SameAccountRetry(t *testing.T) {
	...
	t.Run("不同账号独立跟踪重试次数", func(t *testing.T) {
		...
		// 账号 200 第一次重试（独立计数）
		action = fs.HandleFailoverError(context.Background(), mock, nil, 200, "openai", err)
		require.Equal(t, FailoverContinue, action)
...
func TestHandleFailoverError_TempUnschedule(t *testing.T) {
	...
	t.Run("重试错误耗尽后调用TempUnschedule_传入正确参数", func(t *testing.T) {
		...
		for i := 0; i < maxSameAccountRetries; i++ {
			fs.HandleFailoverError(context.Background(), mock, nil, 42, "openai", err)
		}
...
func TestHandleFailoverError_ContextCanceled(t *testing.T) {
	t.Run("同账号重试sleep期间context取消", func(t *testing.T) {
		...
		start := time.Now()
		action := fs.HandleFailoverError(ctx, mock, nil, 100, "openai", err)
		elapsed := time.Since(start)
	...
	t.Run("Antigravity延迟期间context取消", func(t *testing.T) {
		...
		start := time.Now()
		action := fs.HandleFailoverError(ctx, mock, nil, 100, service.PlatformAntigravity, err)
		elapsed := time.Since(start)
...
func TestHandleFailoverError_FailedAccountIDs(t *testing.T) {
	t.Run("切换时添加到失败列表", func(t *testing.T) {
		...
		fs.HandleFailoverError(context.Background(), mock, nil, 100, "openai", newTestFailoverErr(500, false, false))
		require.Contains(t, fs.FailedAccountIDs, int64(100))

		fs.HandleFailoverError(context.Background(), mock, nil, 200, "openai", newTestFailoverErr(502, false, false))
		require.Contains(t, fs.FailedAccountIDs, int64(200))
	...
	t.Run("耗尽时也添加到失败列表", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(context.Background(), mock, nil, 100, "openai", newTestFailoverErr(500, false, false))
		require.Equal(t, FailoverExhausted, action)
		require.Contains(t, fs.FailedAccountIDs, int64(100))
	})
	...
	t.Run("同账号重试期间不添加到失败列表", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(context.Background(), mock, nil, 100, "openai", newTestFailoverErr(400, true, false))
		require.Equal(t, FailoverContinue, action)
...
func TestHandleFailoverError_IntegrationScenario(t *testing.T) {
	t.Run("模拟完整failover流程_多账号混合重试与切换", func(t *testing.T) {
		...
		for i := 0; i < maxSameAccountRetries; i++ {
			action := fs.HandleFailoverError(context.Background(), mock, nil, 100, "openai", retryErr)
			require.Equal(t, FailoverContinue, action)
		}
		...
		action = fs.HandleFailoverError(context.Background(), mock, nil, 200, "openai", switchErr)
		require.Equal(t, FailoverContinue, action)
		require.Equal(t, 2, fs.SwitchCount)

		// 4. 账号 300 遇到不可重试错误 → 再切换
		action = fs.HandleFailoverError(context.Background(), mock, nil, 300, "openai", switchErr)
		require.Equal(t, FailoverContinue, action)
		require.Equal(t, 3, fs.SwitchCount)

		// 5. 账号 400 → 已耗尽 (SwitchCount=3 >= MaxSwitches=3)
		action = fs.HandleFailoverError(context.Background(), mock, nil, 400, "openai", switchErr)
		require.Equal(t, FailoverExhausted, action)
	...
	t.Run("模拟Antigravity平台完整流程", func(t *testing.T) {
		...
		start = time.Now()
		action = fs.HandleFailoverError(context.Background(), mock, nil, 300, service.PlatformAntigravity, err)
		elapsed = time.Since(start)
		require.Equal(t, FailoverExhausted, action)
	...
	t.Run("ForceCacheBilling通过错误标志设置", func(t *testing.T) {
		...
		fs.HandleFailoverError(context.Background(), mock, nil, 300, "openai", err3)
...
func TestHandleFailoverError_EdgeCases(t *testing.T) {
	...
	t.Run("AccountID为0也能正常跟踪", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(context.Background(), mock, nil, 0, "openai", err)
		require.Equal(t, FailoverContinue, action)
	...
	t.Run("负AccountID也能正常跟踪", func(t *testing.T) {
		...
		action := fs.HandleFailoverError(context.Background(), mock, nil, -1, "openai", err)
		require.Equal(t, FailoverContinue, action)
	...
	t.Run("空平台名称不触发Antigravity延迟", func(t *testing.T) {
		...
		start := time.Now()
		action := fs.HandleFailoverError(context.Background(), mock, nil, 100, "", err)
		elapsed := time.Since(start)
